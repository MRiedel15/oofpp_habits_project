...
def load_pre_habit_data(db):
    """
    loading predefined habits into habit table
    :param db: an initialized sqlite3 database connection
    :return: predefined data loaded into database
    """
    for elem in init_counter_list:
        add_habit(db, elem[0], elem[1], elem[2])


def load_pre_tracker_data(db):
    """
    loading predefined tracker data into tracker table
    :param db: an initialized sqlite3 database connection
    :return: predefined data loaded into database
    """
...
def edit_habit(db, name, description, periodicity):
    ...
    cur = db.cursor()
    cur.execute(edit, (description, periodicity, name))
    db.commit()
